...
export class UserService {
    ...
    async getAllUsers(): Promise<Array<User>> {
        const query = 'SELECT * FROM utilisateur where status != 0';
        ...
        if (result.length === 0) {
            ...
            for (let i = 0; i < result.length; i++) {
                ...
                const userInfo = new User(result[i].id_utilisateur, result[i].nom, result[i].prenom, result[i].adresse, result[i].contact, result[i].email, result[i].mdp, result[i].numero_matricule, userDepartement, userRole, result[i].status)
                users.push(userInfo)
            }
            return users;
        }
